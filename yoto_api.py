...
# Prefer centralized paths when available (new src/ layout). Fall back to
# literal filenames/dirs for legacy execution.
try:
...
class YotoAPI:
    ...
    TOKEN_FILE = paths.TOKENS_FILE if paths and getattr(paths, 'TOKENS_FILE', None) else Path("tokens.json")
    CACHE_FILE = paths.API_CACHE_FILE if paths and getattr(paths, 'API_CACHE_FILE', None) else Path(".yoto_api_cache.json")
    UPLOAD_ICON_CACHE_FILE = paths.UPLOAD_ICON_CACHE_FILE if paths and getattr(paths, 'UPLOAD_ICON_CACHE_FILE', None) else Path(".yoto_icon_upload_cache.json")
    OFFICIAL_ICON_CACHE_DIR = paths.OFFICIAL_ICON_CACHE_DIR if paths and getattr(paths, 'OFFICIAL_ICON_CACHE_DIR', None) else Path(".yoto_icon_cache")
    YOTOICONS_CACHE_DIR: Path = paths.YOTOICONS_CACHE_DIR if paths and getattr(paths, 'YOTOICONS_CACHE_DIR', None) else Path(".yotoicons_cache")
    VERSIONS_DIR: Path = paths.VERSIONS_DIR if paths and getattr(paths, 'VERSIONS_DIR', None) else Path(".card_versions")
