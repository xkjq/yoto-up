...
class IconReplaceDialog:
        ...
        try:
            ...
            if cp:
                try:
                    ...
                    logger.debug(
                        f"upload: direct cache_path upload attempt failed for {cp}",
                        exc_info=True,
                    )

            # If png_base64 present, write to temp file and retry with cache_path
            b64 = (
                payload.get("png_base64")
                or payload.get("pngBase64")
                or payload.get("pngbase64")
            )
            title = (
                payload.get("title") or (payload.get("metadata") or {}).get("title")
            ...
            if b64:
                try:
                    ...
                    try:
                        ...
                        res = self.api.upload_yotoicons_icon_to_yoto_api(
                            {"cache_path": tmp_path, "title": title}
                        )
                        return res
                    except Exception:
                        ...
                        try:
                            ...
                            logger.exception(
                                "upload: failed to upload png_base64 payload"
                            )
                            return None
                    finally:
                        try:
                            ...
                            logger.error(
                                f"upload: failed to delete temp file {tmp_path}"
                            )
                except Exception:
    ...
    def open(self):
        ...
        if self.kind == "chapter":
            apply_to_first_track = ft.Checkbox(
                label="Also replace first track icon", value=True
            )
        else:
            apply_to_first_track = None
        max_searches_field = ft.TextField(
            label="Max extra searches", value="2", width=120
        )
                ...
                try:
                    ...
                    if not icons:
                        results_list.controls.append(
                            ft.Text(value="No icons found", selectable=True)
                        )
                    else:
                        for icon in icons:
                            img_src = None
                            ...
                            try:
                                if icon.get("mediaId"):
                                    p = self.api.get_icon_cache_path(
                                        f"yoto:#{icon.get('mediaId')}"
                                    )
                                    if p and Path(p).exists():
                                        img_src = p
                                if (
                                    not img_src
                                    and icon.get("cache_path")
                                    and Path(icon.get("cache_path")).exists()
                                ):
                            ...
                            def use_icon(ev2, icon=icon):
                                def use_worker():
                                    ...
                                    if not media_id and "id" in icon:
                                        ...
                                        media_id = (
                                            uploaded.get("mediaId")
                                            if isinstance(uploaded, dict)
                                            else None
                                        )
                                    if not media_id:
                                        self.show_snack(
                                            "Selected icon could not be uploaded or has no media id",
                                            error=True,
                                        )
                                        return
                                    if self.kind == "chapter":
                                        ...
                                        try:
                                            if (
                                                apply_to_first_track
                                                and apply_to_first_track.value
                                            ):
                                                if (
                                                    getattr(target_ch, "tracks", None)
                            ...
                            if url_src is not None:
                                ...
                                row_children.append(
                                    ft.GestureDetector(content=img, on_tap=use_icon)
                                )
                            elif img_src is not None:
                                try:
                                    img = ft.Image(
                                        src=get_base64_from_path(img_src),
                                        width=48,
                                        height=48,
                                    )
                                    row_children.append(
                                    ...
                                    placeholder = ft.Container(
                                        width=48, height=48, bgcolor=ft.Colors.GREY_200
                                    )
                                    row_children.append(
                                        ft.GestureDetector(
                                            content=placeholder,
                            ...
                            title_text = (
                                icon.get("title")
                                or icon.get("id")
                                or icon.get("displayIconId")
                                or str(icon)
                            )
                            row_children.append(
                                ft.Column(
                                    controls=[
                                        ft.Text(value=title_text, selectable=True),
                                        ft.Text(
                                            value=", ".join(icon.get("tags", [])[:5])
                                            if icon.get("tags")
                                            else ""
                                        ),
                                    ]
                            ...
                            row_children.append(
                                ft.Button(content="Use", on_click=use_icon)
                            )
                            results_list.controls.append(
                                ft.Row(
            ...
            try:
                ...
                if not saved_dir.exists():
                    try:
                        ...
                        saved_icons_list.controls.append(
                            ft.Text(value="No saved_icons folder")
                        )
                        return
                # if empty, seed some basic stamps
                try:
                    has_files = any(
                        p.suffix.lower() in (".png", ".json")
                ...
                if not files:
                    saved_icons_list.controls.append(
                        ft.Text(value="No saved icons found")
                    )
                    return
                for p in files:
                            ...
                            def _worker():
                                try:
                                    ...
                                    if pth.suffix.lower() == ".json":
                                        try:
                                            obj = json.loads(
                                                pth.read_text(encoding="utf-8") or "{}"
                                            )
                                            ...
                                            pass
                                    if (
                                        icon_payload is None
                                        and pth.suffix.lower() == ".png"
                                    ):
                                        try:
                                            import base64
                                    ...
                                    if not icon_payload:
                                        self.show_snack(
                                            "Cannot upload this saved icon (no image data)",
                                            True,
                                        )
                                        return
                                    # upload via robust helper
                                    uploaded = self._upload_icon_payload(icon_payload)
                                    if not uploaded or not (
                                        isinstance(uploaded, dict)
                                        and uploaded.get("mediaId")
                                    ):
                                        self.show_snack(
                                            "Upload failed or returned no mediaId", True
                                        )
                                        return
                    ...
                    try:
                        preview = ft.Image(
                            src=get_base64_from_path(p), width=48, height=48
                        )
                    except Exception:
                    ...
                    row = ft.Row(
                        controls=[
                            ft.Column(controls=[preview]),
                            ft.Column(
                                controls=[ft.Text(value=p.name, selectable=True)]
                            ),
                            ft.Row(
                                controls=[
                                    ft.Button(content="Edit", on_click=make_edit(p)),
                                    ft.Button(content="Use", on_click=make_use(p)),
                                ]
                            ),
                        ],
                        alignment=ft.MainAxisAlignment.SPACE_BETWEEN,
                ...
                saved_icons_list.controls.append(
                    ft.Text(value=f"Failed to read saved icons: {e}")
                )
            self.page.update()
        ...
        def update_preview(marked_path):
            ...
            try:
                ...
                try:
                    abs_path = (
                        marked_path
                        if os.path.isabs(marked_path)
                        else os.path.abspath(marked_path)
                    )
                except Exception:
                ...
                try:
                    if abs_path and (
                        ...
                        b64_data = get_base64_from_path(abs_path)
                        preview_image.src = f"data:image/png;base64,{b64_data}"
                        show_it = True
                    else:
                        preview_image.src = ""
                        show_it = False
                ...
                preview_name.value = (
                    os.path.basename(marked_path) if marked_path else ""
                )
        ...
        def use_selected_icon(ev=None):
            ...
            def _worker():
                try:
                    ...
                    if pth.suffix.lower() == ".json":
                        try:
                            ...
                            if isinstance(obj, dict):
                                if obj.get("png_base64"):
                                    icon_payload = {
                                        "png_base64": obj.get("png_base64"),
                                        "title": (obj.get("metadata") or {}).get(
                                            "title"
                                        ),
                                    }
                                elif obj.get("pixels"):
                                        ...
                                        for yy in range(h):
                                            for xx in range(w):
                                                ...
                                                if isinstance(c, str) and c.startswith(
                                                    ...
                                                    img.putpixel(
                                                        (xx, yy), (255, 255, 255, 255)
                                                    )
                                        buf = io.BytesIO()
                                        img.save(buf, format="PNG")
                                        icon_payload = {
                                            "png_base64": _b64.b64encode(
                                                buf.getvalue()
                                            ).decode("ascii")
                                        }
                                    except Exception:
                    ...
                    if not icon_payload:
                        self.show_snack(
                            "Marked icon cannot be uploaded (no image data)", True
                        )
                        return

                    uploaded = self._upload_icon_payload(icon_payload)
                    if not uploaded or not (
                        isinstance(uploaded, dict) and uploaded.get("mediaId")
                    ):
        ...
        search_content = ft.Column(
            controls=[
                ft.Row(
                    controls=[
                        search_field,
                        ft.Row(controls=[search_btn, search_progress, search_status]),
                    ]
                ),
                    ...
                    lambda: ft.Row(
                        controls=[
                            ...
                            for c in (
                                ...
                                + [max_searches_field, top_n_field]
                            )
                        ]
                    )
                )(),
        ...
        my_icons_content = ft.Column(
            controls=[saved_icons_list], width=900, expand=True
        )
        my_icons_content.visible = True

        all_tab_labels = [
            ft.Tab(label="Search"),
            ft.Tab(label="My Icons"),
        ]
        all_tab_content = [
            search_content,
            my_icons_content,
        ...
        use_selected_btn = ft.TextButton(
            content="Selected icon", on_click=use_selected_icon
        )
