...
# Prefer centralized paths when available (src layout). Fall back to the
# previous environment-variable based behavior for legacy execution.
try:
    import yoto_up.paths as paths
except Exception:
    paths = None

if paths and getattr(paths, 'OFFICIAL_ICON_CACHE_DIR', None):
    YOTO_ICON_CACHE_DIR = paths.OFFICIAL_ICON_CACHE_DIR
else:
    _flet = os.getenv("FLET_APP_STORAGE_DATA")
    _base = Path(_flet) if _flet else Path('.')
    YOTO_ICON_CACHE_DIR = _base / ".yoto_icon_cache"

if paths and getattr(paths, 'YOTOICONS_CACHE_DIR', None):
    YOTOICONS_CACHE_DIR = paths.YOTOICONS_CACHE_DIR
else:
    _flet2 = os.getenv("FLET_APP_STORAGE_DATA")
    _base2 = Path(_flet2) if _flet2 else Path('.')
    YOTOICONS_CACHE_DIR = _base2 / ".yotoicons_cache"
